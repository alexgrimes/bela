...
// This is for audio inputs and outputs but you could adjust this code to use it for analog inputs
// and outputs from audioRead and audioWrite to analogRead and analogWrite, change to count the number 
// analog frames rather than audio frames and use the analog sample rate rather than the audio sample 
// rate.
...
void render(BelaContext *context, void *userData)
{
	...
	for(unsigned int n = 0; n < context->audioFrames; n++) {
		...
		if(++gTestCounter >= gTestInterval) {
			...
			//check if we got a response to the last pusle, and if not, print a message
			if(gInTest) {
				rt_printf("Pusle timeout\n");
			}
			
			//initialize the next pulse 
			gInTest = true; 
		}
		...
		if(gInTest) {
			if(!gInputAboveThreshold && input >= kInputHiThreshold) {
				...
				// to remove the baked in latency
