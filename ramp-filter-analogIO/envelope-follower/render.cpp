...
// Last state of the envelope
float gLastEnvelopeSample = 0;
...
void render(BelaContext *context, void *userData)
{
	...
	float attackCoeff = pow(exp(-1.0 / attackTime), 1.0 / context->audioSampleRate);
	float decayCoeff = pow(exp(-1.0 / decayTime), 1.0 / context->audioSampleRate);
    ...
    for(unsigned int n = 0; n < context->audioFrames; n++) {
		...
		float envelopeInput = fabsf(in);
		if(in > gLastEnvelopeSample) {		//x[n] > y[n-1]: attack phase
			envelopeOutput = attackCoeff * gLastEnvelopeSample + (1 - attackCoeff) * envelopeInput;
		}
		else {								//x[n] <= y[n-1]: decay phase
    	...
    	float frequency = baseFrequency + envelopeOutput + freqSensitivity;
